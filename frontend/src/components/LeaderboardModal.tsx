...
export default function LeaderboardModal({ apiBase, onClose }: LeaderboardModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="bg-neutral-800 rounded-2xl shadow-2xl p-6 max-w-3xl w-full mx-4 relative max-h-[85vh] flex flex-col">
        ...
        <h2 className="text-2xl font-bold mb-1 text-center text-white">
          Global <span className="font-heading text-amber-400">Leaderboard</span>
        </h2>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 bg-neutral-800">
                <tr className="text-neutral-400 border-b border-neutral-700">
                  ...
                  <th className="py-2 px-2 text-center hidden sm:table-cell">Depth</th>
                  <th className="py-2 px-2 text-right hidden sm:table-cell">Time</th>
                  <th className="py-2 px-2 text-left hidden md:table-cell">Location</th>
                </tr>
              </thead>
              <tbody>
                {entries.map((entry, i) => (
                  <tr
                    ...
                    className={`border-b border-neutral-700/50 hover:bg-neutral-700/30 transition-colors
                      ${i === 0 ? 'text-amber-400 font-semibold' : i < 3 ? 'text-amber-200' : 'text-neutral-300'}`}
                  >
                    <td className="py-2 px-2">
                      {i === 0 ? '\uD83E\uDD47' : i === 1 ? '\uD83E\uDD48' : i === 2 ? '\uD83E\uDD49' : i + 1}
                    </td>
                    <td className="py-2 px-2 font-medium">{entry.player_name}</td>
                    <td className="py-2 px-2 text-right tabular-nums">{entry.score.toLocaleString()}</td>
                    <td className="py-2 px-2 text-right tabular-nums">{entry.rating}</td>
