...
int is_move_interesting(int **board, int x, int y, int stones_on_board,
  ...
  for (int i = max(0, x - MAX_RADIUS); i <= min(board_size - 1, x + MAX_RADIUS);
       i++) {
    for (int j = max(0, y - MAX_RADIUS);
         j <= min(board_size - 1, y + MAX_RADIUS); j++) {
      if (board[i][j] != AI_CELL_EMPTY) {
...
void find_best_ai_move(game_state_t *game, int *best_x, int *best_y) {
  ...
  *best_y = -1;
  ...
  // Generate and sort moves using optimized method
  move_t moves[361]; // Max for 19x19 board
  int move_count = generate_moves_optimized(game, game->board, moves,
                                            AI_CELL_NAUGHTS, game->max_depth);

  // Check for immediate winning moves first
  for (int i = 0; i < move_count; i++) {
    if (evaluate_threat_fast(game->board, moves[i].x, moves[i].y,
                             AI_CELL_NAUGHTS, game->board_size) >= 100000) {
      *best_x = moves[i].x;
      *best_y = moves[i].y;
      snprintf(game->ai_status_message, sizeof(game->ai_status_message),
               "%s%s%s It's a checkmate ;-)", COLOR_BLUE, "O", COLOR_RESET);
