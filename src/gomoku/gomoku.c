...
int count_squares(int value, int player, int *last_square, int *hole_count,
                  int *square_count, int *contiguous_square_count,
                  int *enemy_count);
...
int other_player(int player) { return -player; }
...
int calc_threat_in_one_dimension(int *row, int player) {
  ...
  for (i = SEARCH_RADIUS + 1, last_square = player;
    ...
    if (count_squares(row[i], player, &last_square, &right_hole_count,
                      &player_square_count, &player_contiguous_square_count,
                      &enemy_count) == RT_BREAK) {
      break;
    }
  }
  ...
  if (player_contiguous_square_count >= NEED_TO_WIN) {
    threat = THREAT_FIVE;
  } else if (player_contiguous_square_count == 4 && right_hole_count > 0 &&
             left_hole_count > 0) {
    threat = THREAT_STRAIGHT_FOUR;
  } else if (player_contiguous_square_count == 4 &&
             (right_hole_count > 0 || left_hole_count > 0)) {
    threat = THREAT_FOUR;
  } else if (player_contiguous_square_count == 3 &&
             (right_hole_count > 0 && left_hole_count > 0)) {
    threat = THREAT_THREE;
  } else if (player_square_count >= 4 &&
             (right_hole_count > 0 || left_hole_count > 0) && total >= 5) {
    threat = THREAT_FOUR_BROKEN;
  } else if (player_square_count >= 3 &&
             (right_hole_count > 0 || left_hole_count > 0) && total >= 5) {
    threat = THREAT_THREE_BROKEN;
  } else if (player_contiguous_square_count >= 2 &&
             (right_hole_count > 0 || left_hole_count > 0) && total >= 4) {
    threat = THREAT_TWO;
...
int calc_combination_threat(int one, int two) {
  ...
  if (one == THREAT_THREE &&
      (two == THREAT_FOUR || two == THREAT_STRAIGHT_FOUR ||
       two == THREAT_FOUR_BROKEN)) {
  ...
  if (one == THREAT_THREE_BROKEN &&
      (two == THREAT_FOUR || two == THREAT_STRAIGHT_FOUR ||
       two == THREAT_FOUR_BROKEN)) {
    return threat_cost[THREAT_THREE_AND_THREE]; // Treat as strong as double
                                                // three
  }
...
void populate_threat_matrix() {
  ...
  threat_cost[THREAT_STRAIGHT_FOUR] =
      50000;                        // Open four = guaranteed win next turn
  ...
  threat_cost[THREAT_THREE_AND_THREE_BROKEN] =
      5000; // Weaker but still dangerous
}
...
int minimax_example(int **board, int size, int depth, int alpha, int beta,
  ...
  if (depth == 0 || has_winner(board, size, ai_player) ||
      has_winner(board, size, other_player(ai_player))) {
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        if (board[i][j] == AI_CELL_EMPTY) {
          ...
          // Recursive call
          int eval = minimax_example(board, size, depth - 1, alpha, beta, 0,
                                     ai_player);
    ...
    for (int i = 0; i < size; i++) {
      for (int j = 0; j < size; j++) {
        if (board[i][j] == AI_CELL_EMPTY) {
          ...
          // Recursive call
          int eval = minimax_example(board, size, depth - 1, alpha, beta, 1,
                                     ai_player);
