...
cli_config_t parse_arguments(int argc, char *argv[]) {
  ...
  while ((c = getopt_long(argc, argv, "d:l:t:b:r:husx:o:", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'd':
...
void print_help(const char *program_name) {
  ...
  printf("  %s-o, --player-o TYPE%s   Player O type: \"human\" or \"ai\" "
         "(default: ai)\n",
         COLOR_YELLOW, COLOR_RESET);
  printf("  %s-d, --depth N%s         The depth of search. Use N for both, or "
         "N:M for\n",
         COLOR_YELLOW, COLOR_RESET);
  printf("                        asymmetric depths (X:O). Examples: '4' or "
         "'4:6'\n");
