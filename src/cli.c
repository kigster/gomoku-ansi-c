...
cli_config_t parse_arguments(int argc, char *argv[]) {
  cli_config_t config = {.board_size = 19,  // Default board size
  ...
  static struct option long_options[] = {{"depth", required_argument, 0, 'd'},
  ...
  while ((c = getopt_long(argc, argv, "d:l:t:b:hus", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'd':
...
void print_help(const char *program_name) {
  ...
  printf("%sFLAGS:%s\n", COLOR_BRIGHT_MAGENTA, COLOR_RESET);
  printf("  %s-d, --depth N%s         The depth of search in the MiniMax "
         "algorithm\n",
