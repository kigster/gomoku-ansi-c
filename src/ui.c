...
void handle_input(game_state_t *game) {
  ...
  case KEY_ENTER:
    if (game->current_player == AI_CELL_CROSSES &&
        is_valid_move(game->board, game->cursor_x, game->cursor_y,
                      game->board_size)) {
      double move_time = end_move_timer(game);
      make_move(game, game->cursor_x, game->cursor_y, AI_CELL_CROSSES,
                move_time, 0);
    }
    break;
  case 'U':
      ...
      if (game->board[i][j] == AI_CELL_EMPTY) {
        ...
        if (is_cursor_here) {
          ...
          if (i == game->last_ai_move_x && j == game->last_ai_move_y) {
            //   printf("%s%s%s", COLOR_O_INVALID, UNICODE_NAUGHTS,
            //   COLOR_RESET);
            printf("%s%s%s", COLOR_RESET, UNICODE_OCCUPIED, COLOR_RESET);
          } else {
...
void draw_status(game_state_t *game) {
  ...
  if (game->current_player == AI_CELL_CROSSES) {
    printf("%s│%s %-*s %s│%s%s\n", prefix, COLOR_YELLOW,
           action_width + control_width + 2, "Current Player : You (X)",
           COLOR_RESET, COLOR_RESET, COLOR_YELLOW);
  } else {
    printf("%s│%s %-*s %s│%s%s\n", prefix, COLOR_BLUE,
           action_width + control_width + 2, "Current Player : Computer (O)",
           COLOR_RESET, COLOR_RESET, COLOR_BLUE);
  }
  ...
  if (game->game_state != GAME_RUNNING) {
    ...
    case GAME_HUMAN_WIN:
      printf("%s%s│ %-*s %s│\n", prefix, COLOR_RESET, box_width - 4,
             "Human wins! Great job!", COLOR_RESET);
      break;
    case GAME_AI_WIN:
      printf("%s%s│ %-*s %s│\n", prefix, COLOR_RESET, box_width - 4,
             "AI wins! Try again!", COLOR_RESET);
      break;
    case GAME_DRAW:
      printf("%s%s│%s %-*s %s│\n", prefix, COLOR_RESET, COLOR_RESET,
    ...
    snprintf(time_summary, sizeof(time_summary),
             "Time: Human: %.1fs | AI: %.1fs", game->total_human_time,
             game->total_ai_time);
    printf("%s%s│ %-*s %s│\n", prefix, COLOR_RESET, box_width - 4, time_summary,
           COLOR_RESET);
