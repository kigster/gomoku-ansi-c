...
void draw_game_history_sidebar(game_state_t *game, int start_row) {
    ...
    printf("\033[%d;%dH%s%sGame History:%s", start_row, sidebar_col, COLOR_BOLD_CROSSES, COLOR_GREEN, COLOR_RESET);
    printf("\033[%d;%dH%sMove Player [Time] (AI positions evaluated)%s", start_row + 1, sidebar_col, COLOR_BOLD_CROSSES, COLOR_RESET);
        ...
        for (int j = 0; j < game->board_size; j++) {
            ...
            if (game->board[i][j] == AI_CELL_EMPTY) {
                if (is_cursor_here) {
                    ...
                    printf("%s%s%s", COLOR_BG_CROSSES, UNICODE_EMPTY, COLOR_RESET);
                }
            } else if (game->board[i][j] == AI_CELL_CROSSES) {
                if (is_cursor_here) {
                    // Human stone with cursor: add grey background
                    printf("%s%s%s%s", COLOR_BG_GREY, COLOR_RED, UNICODE_CROSSES, COLOR_RESET);
                } else {
                    // Human stone without cursor: normal red
                    printf("%s%s%s", COLOR_RED, UNICODE_CROSSES, COLOR_RESET);
                }
            } else { // AI_CELL_NAUGHTS
                if (is_cursor_here) {
                    // AI stone with cursor: add grey background
                    if (i == game->last_ai_move_x && j == game->last_ai_move_y) {
                        printf("%s%s%s%s", COLOR_BG_GREY, COLOR_BRIGHT_BLUE, UNICODE_NAUGHTS, COLOR_RESET);
                    } else {
                        printf("%s%s%s%s", COLOR_BG_GREY, COLOR_BLUE, UNICODE_NAUGHTS, COLOR_RESET);
                    }
                } else {
                    // AI stone without cursor: normal highlighting
                    if (i == game->last_ai_move_x && j == game->last_ai_move_y) {
                        printf("%s%s%s", COLOR_BRIGHT_BLUE, UNICODE_NAUGHTS, COLOR_RESET);
                    } else {
                        printf("%s%s%s", COLOR_BLUE, UNICODE_NAUGHTS, COLOR_RESET);
                    }
                }
            }
        }
...
void draw_status(game_state_t *game) {
    ...
    if (game->current_player == AI_CELL_CROSSES) {
        printf("%s│ %-*s %s│%s%s\n", COLOR_YELLOW, box_width - 4,
               "Current Player : X (Human)", COLOR_BOLD_CROSSES, COLOR_RESET, COLOR_YELLOW);
    } else {
        printf("%s│ %-*s %s│%s%s\n", COLOR_BLUE, box_width - 4,
               "Current Player : O (AI)", COLOR_BOLD_CROSSES, COLOR_RESET, COLOR_BLUE);
    }
    ...
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, position_str, COLOR_BOLD_CROSSES, COLOR_RESET);
    ...
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, difficulty_str, COLOR_BOLD_CROSSES, COLOR_RESET);

    memset(difficulty_str, 0, sizeof(difficulty_str));

    snprintf(difficulty_str, sizeof(difficulty_str),
             "Search Depth   : %d", game->max_depth);
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, difficulty_str, COLOR_BOLD_CROSSES, COLOR_RESET);
    
    // Separator line
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, "", COLOR_BOLD_CROSSES, COLOR_RESET);
    
    // Controls header
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, "Controls :", COLOR_BOLD_CROSSES, COLOR_RESET);
    
    // Control instructions
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, " Arrow Keys    - Move cursor", COLOR_BOLD_CROSSES, COLOR_RESET);
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, " Space / Enter - Make move", COLOR_BOLD_CROSSES, COLOR_RESET);
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, " U             - Undo last move pair", COLOR_BOLD_CROSSES, COLOR_RESET);
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, " ?             - Show game rules", COLOR_BOLD_CROSSES, COLOR_RESET);
    printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
           box_width - 4, " ESC           - Quit game", COLOR_BOLD_CROSSES, COLOR_RESET);
    ...
    if (strlen(game->ai_status_message) > 0) {
        ...
        *clean_ptr = '\0';
        ...
        printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
               box_width - 4, clean_message, COLOR_BOLD_CROSSES, COLOR_RESET);
    }
    ...
    if (game->game_state != GAME_RUNNING) {
        ...
        case GAME_HUMAN_WIN:
            printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_YELLOW, 
                   box_width - 4, "Human wins! Great job!", COLOR_BOLD_CROSSES, COLOR_RESET);
            break;
        case GAME_AI_WIN:
            printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_BLUE, 
                   box_width - 4, "AI wins! Try again!", COLOR_BOLD_CROSSES, COLOR_RESET);
            break;
        case GAME_DRAW:
            printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
                   box_width - 4, "The Game is a draw!", COLOR_BOLD_CROSSES, COLOR_RESET);
            break;
        }
        ...
        printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
               box_width - 4, time_summary, COLOR_BOLD_CROSSES, COLOR_RESET);
        
        printf("%s│%s %-*s %s│%s\n", COLOR_BOLD_CROSSES, COLOR_RESET, 
               box_width - 4, "Press any key to exit...", COLOR_BOLD_CROSSES, COLOR_RESET);
    }
