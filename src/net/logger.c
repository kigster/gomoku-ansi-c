...
static long getCurrentProcessID(void) {
#if defined(_WIN32) || defined(_WIN64)
...
static long vflog(FILE *fp, char levelc, const char *timestamp, long pid,
  ...
  if ((size = fprintf(fp, "%c %s [%ld] %s:%d: ", levelc, timestamp, pid, file,
                      line)) > 0) {
    totalsize += size;
  }
...
void logger_log(LogLevel level, const char *file, int line, const char *fmt,
  ...
  if (hasFlag(s_logger, kConsoleLogger)) {
    va_start(carg, fmt);
    vflog(s_clog.output, levelc, timestamp, pid, file, line, fmt, carg,
  ...
  if (hasFlag(s_logger, kFileLogger)) {
    if (rotateLogFiles()) {
      ...
      s_flog.currentFileSize +=
          vflog(s_flog.output, levelc, timestamp, pid, file, line, fmt, farg,
